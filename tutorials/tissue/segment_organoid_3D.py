...
if __name__=="__main__":
    ...
    to generate 3D segmentation using the direct method with all three orthoviews for a single organoid example from the EmbedSeg paper.
    We will also show how to compute the AP curve for benchmarking and compare with/without systematic relabeling. 
    ...
    # Load the mouse organoid image from EmbedSeg dataset. 
    imgfile = '../../example_data/tissue/organoid/X001.tif'
    ...
    """
    This image is not isotropic. Since we are using a pretrained model which assumes isotropic, we need to resize accordingly. This means setting the preprocess_params
    """
    import skimage.exposure as skexposure
    # img = skexposure.adjust_gamma(img, gamma=0.5)
    
    
    px_res = 0.1733 # the ratio of xy vs z. (first axis) 
    ...
    preprocess_params['bg_ds'] = 8. # tuning this can help increase contrast
    ...
    uSegment3D_fio.save_segmentation(os.path.join(savecellfolder,
                                                  'uSegment3D_organoid_labels.tif'), segmentation3D)
    ...
    uSegment3D_fio.write_pickle(os.path.join(savecellfolder,
                                                  'uSegment3D_organoid_3Dcombined_probs_and_gradients'), 
    ...
    # save this out 
    uSegment3D_fio.save_segmentation(os.path.join(savecellfolder,
                                                  'uSegment3D_organoid_labels_postprocess.tif'), segmentation3D_filt)
    ...
    ref_segmentfile = '../../example_data/tissue/organoid/X001_labels.tif'
    ref_segmentation = skio.imread(ref_segmentfile)
    
    # we need to resize this to the same size as the predicted
    import skimage.transform as sktform 
    ref_segmentation = sktform.resize(ref_segmentation, output_shape=segmentation3D_filt.shape, 
    ...
    """
    Note: we do need to ensure evaluation of the same foreground in this case, because the reference is only defined for the central organoid. 
    """
    plt.figure(figsize=(10,10))
    
    plt.subplot(331)
    plt.title('Max proj of preprocessed')
    plt.imshow(img_preprocess.max(axis=0), cmap='gray'); plt.grid('off'); plt.axis('off')
    plt.subplot(332)
    plt.imshow(img_preprocess.max(axis=1), cmap='gray'); plt.grid('off'); plt.axis('off')
    plt.subplot(333)
    plt.imshow(img_preprocess.max(axis=2), cmap='gray'); plt.grid('off'); plt.axis('off')
    
    plt.subplot(334)
    plt.title('Max proj of ref. segmentation')
    plt.imshow(ref_segmentation_color.max(axis=0)); plt.grid('off'); plt.axis('off')
    plt.subplot(335)
    plt.imshow(ref_segmentation_color.max(axis=1)); plt.grid('off'); plt.axis('off')
    plt.subplot(336)
    plt.imshow(ref_segmentation_color.max(axis=2)); plt.grid('off'); plt.axis('off')
    
    plt.subplot(337)
    plt.title('Max proj of pred. segmentation')
    plt.imshow(pred_segmentation_color.max(axis=0)); plt.grid('off'); plt.axis('off')
    plt.subplot(338)
    plt.imshow(pred_segmentation_color.max(axis=1)); plt.grid('off'); plt.axis('off')
    plt.subplot(339)
    plt.imshow(pred_segmentation_color.max(axis=2)); plt.grid('off'); plt.axis('off')
    plt.savefig(os.path.join(savecellfolder,
                             'final-segmentation_vs_reference_max-projection.png'), dpi=300, bbox_inches='tight')
    plt.show(block=False)
    plt.close('all')
    ...
    """
    compare reference and predicted, remodifying the predicted in order to restrict to overlapping component with reference, else AP will be artificially low. 
    """
    segmentation3D_filt_for_eval, (pred_regions, coverage_regions_with_ref) = uSegment3D_metrics.get_valid_eval_region_segmentation(ref_segmentation, 
                                                                                                                                  segmentation3D_filt, 
                                                                                                                                  min_coverage=0.01)
    
    pred_segmentation3D_filt_for_eval_color = uSegment3D_plotting.color_segmentation(segmentation3D_filt_for_eval, 
                                                                                     cmapname='Spectral', # use the  
                                                                                     n_colors=16)
    ...
    plt.subplot(337)
    plt.title('Max proj of pred. segmentation restricted to overlapping with ref.')
    plt.imshow(pred_segmentation3D_filt_for_eval_color.max(axis=0)); plt.grid('off'); plt.axis('off')
    plt.subplot(338)
    plt.imshow(pred_segmentation3D_filt_for_eval_color.max(axis=1)); plt.grid('off'); plt.axis('off')
    plt.subplot(339)
    plt.imshow(pred_segmentation3D_filt_for_eval_color.max(axis=2)); plt.grid('off'); plt.axis('off')
    plt.savefig(os.path.join(savecellfolder,
                             'final-segmentation_vs_reference_max-projection_restrict-to-ref.png'), dpi=300, bbox_inches='tight')
    plt.show(block=False)
    plt.close('all')
    ...
    predict_labels_relabel = uSegment3D_segment.reorder_labels_3D(segmentation3D_filt_for_eval)
    ...
    """
    fg_binary = segmentation3D_filt_for_eval >0 # derive the current
    
    img_resize = sktform.resize(img, 
                                output_shape=segmentation3D_filt.shape, 
                                order=1, 
                                preserve_range=True)
    
    # use the original image with normalized intensity as guide, with gamma correction 
    guide = uSegment3D_filters.normalize(img_resize,
    ...
    """
    The outline is already very good, so again no real difference. 
    """
