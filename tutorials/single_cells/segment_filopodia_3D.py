...
if __name__=="__main__":
    ...
    to generate 3D segmentation for a single cell with surface filopodia.
    ...
    """
    we will run segmentation downsampled to better resonate with the training of cellpose at diameter = 30, also it makes it easier to capture a much larger shape. 
    """
    ...
    # we can then run the default preprocessing process in uSegment3D. This process is adapted to multichannel images. therefore for single-channel we need to squeeze the output
    ...
    cellpose_segment_params['cellpose_modelname'] = 'cyto2'
    cellpose_segment_params['diam_range'] = np.arange(10,121,2.5) # potentially increase the range for filopodia
    ...
    # # reduce the default number of levels from 3 to 2 to capture weaker cell probabilities
    aggregation_params['combine_cell_probs']['threshold_level'] = 0 # choose the 1st threshold.
    aggregation_params['combine_cell_probs']['threshold_n_levels'] = 2 # use two levels to threshold 
    ...
    uSegment3D_fio.write_pickle(os.path.join(savecellfolder,
                                                  'uSegment3D_filopodia_3Dcombined_probs_and_gradients'), 
    ...
    label_diffusion_params['diffusion']['refine_iters'] = 15 # use lower amount of iterations, since base segmentation is quite close. 
    label_diffusion_params['diffusion']['refine_alpha'] = 0.5  
    label_diffusion_params['diffusion']['refine_clamp'] = 0.75 # the extent we retrain the starting segmentation 
    ...
    uSegment3D_fio.save_segmentation(os.path.join(savecellfolder,
                                                  'uSegment3D_filopodia_labels_postprocess-diffuse.tif'), segmentation3D_filt_diffuse)
    ...
    guided_filter_params['ridge_filter']['sigmas'] = [1.5]
    
    # we can adjust the radius to try to capture more detail 
    guided_filter_params['guide_filter']['radius'] = 75 # drop lower (this should be approx size of protrusions) 
    ...
    guided_filter_params['guide_filter']['base_erode'] = 5
    guided_filter_params['guide_filter']['collision_erode'] = 1
    guided_filter_params['guide_filter']['collision_close'] = 1
    ...
    uSegment3D_fio.save_segmentation(os.path.join(savecellfolder,
                                                  'uSegment3D_filopodia_labels_postprocess-diffuse-guided_filter.tif'), segmentation3D_filt_guide)
    ...
    tmp = cell_surface_mesh.export(os.path.join(savecellfolder,
                                                  'uSegment3D_filopodia_labels_postprocess-diffuse-guided_filter_surface-mesh.obj'))
